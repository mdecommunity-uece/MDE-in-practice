 ...
 * @generated
 */
public class ActivitydiagramValidationProvider {
	...
	public static void runWithConstraints(TransactionalEditingDomain editingDomain, Runnable operation) {
		...
		if (editingDomain != null) {
			try {
				...
				ActivitydiagramDiagramEditorPlugin.getInstance().logError("Validation failed", e); //$NON-NLS-1$
			}
		} else {
			task.run();
		}
	...
	static boolean isInDefaultEditorContext(Object object) {
		...
		if (object instanceof View) {
			return constraintsActive && ActivityDiagramEditPart.MODEL_ID
					.equals(ActivitydiagramVisualIDRegistry.getModelID((View) object));
		}
		return true;
	}
